...
- [~] Météo (`scripts/world/weather_layer.gd` + teinte d'orage dans `background_layer.gd`) : pluie (intensité selon le palier), éclair (déclenché par le niveau d'orage et le bonus de l'Arbre du Chaos, même logique que `maybeTriggerLightning()`), ciel/sol qui s'assombrissent avec l'orage. **Poussé mais pas encore vérifié.** Pas encore fait : inclinaison du vent sur la pluie, nuages, rafales de vent, assombrissement en dégradé + vignette (détails visuels, pas la météo elle-même) — tout ça dépend du moteur de vent, prochaine étape.
- [ ] Physique du vent (arbres qui plient/tombent/repoussent, maisons qui se fissurent/perdent leur toit/s'effondrent, débris, nuages qui dérivent) et fumée des cheminées
