...
extends Button
## Button subclass for ChaosTreeOverlay's header and inspector buttons —
## adds a hand-drawn bevel on top of Button's own normal StyleBoxFlat/text
## rendering via `_draw()` rather than replacing it.
##
...
##
## Used to also draw a custom padlock icon for the tree nodes' locked
## state, back when every button here was a TreeNodeButton. The tree nodes
## moved to real StyleBoxTexture crops from the reference (see
## ChaosTreeOverlay's own header) — node_locked.png already has a padlock
## painted in, so that drawn version and the tree nodes' own use of this
## class are both gone; only the header/inspector buttons, which still
## have no matching reference crop, keep the bevel treatment.
